...
class MediaViewer(Gtk.Box):
    ...
    def build_ui(self):
        ...
        add_tag_btn.connect("clicked", lambda x: self.on_add_tag_activate(self.tag_entry))
    ...
    def cleanup_video(self):
        ...
        if self.video_widget is not None:
            media_stream = self.video_widget.get_media_stream()
            if media_stream:
    ...
    def load_item_details(self):
        ...
        if is_video:
            ...
            # Embedded Gtk.Video — GStreamer-backed inline player with native
    ...
    def on_open_in_celluloid(self, btn):
        ...
        if self.video_widget:
            stream = self.video_widget.get_media_stream()
            if stream:
                stream.pause()
        try:
    ...
    def populate_details_list(self, mime):
        ...
        details = [
            ...
            ("Original Path", self.item['original_relative_path'], "document-open-symbolic"),
            ("File Size", self.format_bytes(self.item['file_size']), "drive-harddisk-symbolic"),
            ("MIME Type", mime, "dialog-information-symbolic"),
            ("Import Date", str(self.item['created_at']), "preferences-system-time-symbolic")
        ]
    ...
    def load_tags(self):
        ...
        try:
            ...
            for tag in tags:
                ...
                del_btn.connect("clicked", lambda x, t_id=tag["id"]: self.remove_tag(t_id))
    ...
    def remove_tag(self, tag_id):
        ...
        try:
            ...
            cursor = conn.cursor()
            cursor.execute("DELETE FROM media_tags WHERE media_id = ? AND tag_id = ?", (self.item["id"], tag_id))
            conn.commit()
            self.load_tags()
        except Exception as e:
    ...
    def on_add_tag_activate(self, entry):
        ...
        try:
            ...
            cursor.execute("INSERT OR IGNORE INTO media_tags (media_id, tag_id) VALUES (?, ?)", (self.item["id"], tag_id))
            conn.commit()
