...
def is_ignored(rel_path: str, ignore_list: List[str]) -> bool:
    ...
    for pattern in ignore_list:
        p = pattern.lower().strip().replace("\\", "/")
        if not p:
            continue
        if p in segments or any(p in seg for seg in segments):
            return True
        # Folder paths picked from the file dialog (e.g. "photos/raw"):
        # ignore anything under that relative path.
        if p in normalized:
            return True
    return False
...
def walk_directory(
    ...
    while stack and scan_state.scanning:
        ...
        try:
            ...
            add_log(
                "warn", f"Could not read directory {current_dir}: {err}", current_dir
            )
            continue
        ...
        for entry in entries:
            ...
            if entry.is_dir(follow_symlinks=False):
                ...
                cursor.execute(
                    "SELECT id, current_relative_path FROM media_items WHERE file_hash = ?",
                    (file_hash,),
                )
                ...
                if existing:
                    add_log(
                        "info",
                        f"Skipping existing media file with hash: {file_hash[:8]}",
                        rel_entry_path,
                    )
                    continue
                ...
                if os.path.exists(target_dest_path):
                    ...
                    while os.path.exists(
                        os.path.join(unknown_dir, f"{base_name}_{counter}{ext_name}")
                    ):
                        counter += 1
                    target_dest_path = os.path.join(
                        unknown_dir, f"{base_name}_{counter}{ext_name}"
                    )

                dest_relative_path = os.path.relpath(target_dest_path, drive_path)
                file_size = entry.stat().st_size
                mime_type = (
                    mimetypes.guess_type(entry.path)[0]
                    or f"application/{ext.lstrip('.')}"
                )
                ...
                add_log(
                    "info",
                    f"Indexed new media item #{scan_state.media_found}",
                    dest_relative_path,
                )
    ...
    add_log(
        "info",
        f"Scan finished. Total checked: {scan_state.files_scanned}, cataloged: {scan_state.media_found}",
    )
